...
public class Classifier implements RecommendAlgo{
	...
	private void doStatistics(int index, ReReEntrance ent) {
		...
		for (int i = 0; i < index; i++) {
			String number = ent.getPrList().get(i).getNumber()+"";
			List<String> reviewers = ent.getCodeReviewers(i);
	...
	public void modelConstruct(int index, ReReEntrance ent) throws Exception{
		try {
			...
			wvtp.process("pr", ent.getPrList().size(), ent);
...
//		readWV(ent.getOwner(), ent.getRepo());
...
//		for (Map.Entry<String, Double> each : entryList) recommendList.add(each.getKey());
//		
//		List<String> truth = ent.getCodeReviewers(i);
//		System.out.print("Classifier: ");
//		for (String ans : truth) {
//			if (recommendList.contains(ans))
//				System.out.print(recommendList.indexOf(ans) + " ");
//			else {
...
//		System.out.println("****"+loss);
	...
	public static void main(String[] args) throws Exception {
		...
		t.recommend(ent.getPrList().size()-2, ent);
