...
import ReviewerRecommendation.Algorithms.RSVD.Blender.Tuple;
...
//		Map<String, List<CommitComment>> cc = ent.getDp().getCommitComments();
//		
...
//				if (com.getPath() != null)
//					file += com.getPath();
//				if (users.contains(user) && items.contains(file)) {
//					int U = users.indexOf(user);
//					int I = items.indexOf(file);
//					if (userToItemsImplicit.containsKey(U) )
//						userToItemsImplicit.get(U).add(I);
//					else {
//						List<Integer> tmp = new ArrayList<Integer>();
//						tmp.add(I);
//						userToItemsImplicit.put(U, tmp);
//					}
//				}
//			}
//		}
...
//				if (i == j) continue;
//				double sim = lcp.similar(files.get(i), files.get(j));
//				res.put(j, sim);
//			}
//			
//			List<Map.Entry<Integer, Double>> tmp = SortMapElement.sortDoubleNH(res);
//			
//			similarFiles.put(i, new ArrayList<Integer>());
//			
//			int cnt = 0;
//			for (Map.Entry<Integer, Double> each : tmp) {
//				similarFiles.get(i).add(each.getKey());
//				cnt ++;
//				if (cnt == K) break;
//			}
//		}
