...
import ReviewerRecommendation.Algorithms.RSVD.BlenderWithOutImplicitFeedBack.Tuple;
...
//	private SVDModel svd;
//	private BasicSVD basicSVD;
...
//			blender = (BlenderWithOutImplicitFeedBack)crossVal.validate("BlenderW");
//			basicSVD = (BasicSVD)crossVal.validate("BasicSVD");
//			svd = (SVDModel)crossVal.validate("SVDModel");
			
//			System.out.println(neigh.getAverRate());
//			for (int i = 0; i < svd.K; i++)
//				System.out.print(svd.puk[1][i] + " ");
		}
	...
	private RatingMatrix constructMatrix(int cur, ReReEntrance ent) {
		...
		for (String rev : reviewerList) {
			...
			for (Integer id : prids) {
				...
				Date nowTime = ent.getPrList().get(id).getCreatedAt();
				for (String file : files) {
